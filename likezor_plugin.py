    ...
    if command == 'register':
        if name:
            ...
            try:
                ...
                try:
                    ...
                    # Check if user has teleegram @username
                    if message.from_user.username:
                        account = (api_account.screen_name,api_account.id,api_account.favourites_count,like_id,message.from_user.username)
                    else:
                        account = (api_account.screen_name,api_account.id,api_account.favourites_count,like_id,message.from_user.id)
                    # Connect to database
                    db_worker = SQLighter(config.database_name)
                    # Check if user already exists in database
                    if db_worker.select_one_account(table,name):
                        bot.send_message(message.chat.id, "Twitter user's been registered already.", reply_to_message_id=message.message_id)
                    else:
                        ...
                        file = "likezor\\" + api_account.screen_name + "_chain.csv"
                        with open(file, "w+") as like_id_chain:
                            for i in range(0,len(api_like)):
                                like_id = api_like[i].id
                                like_id_chain.write(str(like_id)+", ")
                        bot.send_message(message.chat.id,"Twitter user "+ api_account.screen_name + " was successfully registered.", reply_to_message_id=message.message_id)
                except:
                    bot.send_message(message.chat.id, "Likezor blocked by {}!".format(api_account.screen_name), reply_to_message_id=message.message_id)
            except:
                bot.send_message(message.chat.id, "No such Twitter user.", reply_to_message_id=message.message_id)
        else:
            bot.send_message(message.chat.id, "Enter twitter username to register.", reply_to_message_id=message.message_id)
    elif command == 'update':
        if name:
            # Connect to Database
            db_worker = SQLighter(config.database_name)
            ...
            if db_worker.select_one_account(table,name):
                ...
                try:
                    ...
                    try:
                        ...
                        db_worker.close()
                        bot.send_message(message.chat.id, "Twitter user {} was successfully updated.".format(name), reply_to_message_id=message.message_id)
                    except:
                        bot.send_message(message.chat.id, "Likezor blocked by {}!".format(api_account.screen_name), reply_to_message_id=message.message_id)
                except:
                        bot.send_message(message.chat.id, "No such Twitter user.", reply_to_message_id=message.message_id)
            else:
                bot.send_message(message.chat.id, "No Twitter user {} registered.".format(name), reply_to_message_id=message.message_id)
            db_worker.close()
        else:
            bot.send_message(message.chat.id, "Enter twitter username to update.", reply_to_message_id=message.message_id)
        ...
        if name:
            ...
            if db_worker.select_one_account(table,name):
                ...
                db_worker.close()
                bot.send_message(message.chat.id,"Twitter user "+ name + " was successfully unregistered.", reply_to_message_id=message.message_id)
            else:
                bot.send_message(message.chat.id, "No Twitter user {} registered.".format(name), reply_to_message_id=message.message_id)
        else:
            bot.send_message(message.chat.id, "Enter twitter username to unregister.", reply_to_message_id=message.message_id)
            ...
            try:
                ...
                try:
                    ...
                    for i in range(0,len(likes_list)):
                        file = 'likezor\\' + name + '.csv'
                        with open(file,'a+', encoding='utf-8') as f:
                            f.write(likes_list[i].created_at + ',' + likes_list[i].id_str + ',' + likes_list[i].user.screen_name + ',' + likes_list[i].text  + '\n')
                    bot.send_document(message.chat.id, open(file, 'r', encoding='utf-8'), reply_to_message_id=message.message_id, timeout=10)
                except:
                    bot.send_message(message.chat.id, "Likezor blocked by {}!".format(api_account.screen_name), reply_to_message_id=message.message_id)
            except:
                bot.send_message(message.chat.id, "No such Twitter user.", reply_to_message_id=message.message_id)
        else:
            bot.send_message(message.chat.id, "Enter twitter username to download likes for.", reply_to_message_id=message.message_id)
        ...
        try:
            if name:
                ...
                if message.from_user.username:
                    ...
                    db_worker.add_subscriber(table,message.from_user.first_name + ' ' + message.from_user.last_name,message.from_user.id,name)
                ...
                if message.from_user.username:
                    ...
                    db_worker.add_subscriber(table,message.from_user.first_name + ' ' + message.from_user.last_name,message.from_user.id,None)
                elif message.chat.first_name:
            ...
            bot.send_message(message.chat.id, 'You have successfully subscribed.', reply_to_message_id=message.message_id)
        except:
                sub_id = db_worker.select_one_subscriber(table,message.from_user.id,)
                bot.send_message(message.chat.id, 'You are already subscribed with an id = {}.'.format(sub_id), reply_to_message_id=message.message_id)
        db_worker.close()
    elif command == 'unsubscribe':
        table = 'subscribers'
        # Connect to database
        db_worker = SQLighter(config.database_name)
        if db_worker.select_one_subscriber(table,message.from_user.id,) and message.from_user.id == db_worker.select_one_subscriber(table,message.from_user.id,)[0][2]:
            db_worker.delete_subscriber(table,message.from_user.id)
            db_worker.close()
            bot.send_message(message.chat.id,"You have successfully unsubscribed.", reply_to_message_id=message.message_id)
        else:
    ...
    for i in range(0, len(accounts)):
        ...
        if api_account.favourites_count > likes_nums[i]:
            ...
            try:
                ...
                print(screen_names[i] + ": db likes: " + str(likes_nums[i]) + "; api likes: " + str(api_account.favourites_count) + "; db like id: " + str(like_ids[i]) + "; api like id: " + api_likes[0].id_str)
            except:
...
t_likezor = threading.Thread(target=likezor_timer, name="Time thread for likezor", args=(60, )) # next check after 5 mins
