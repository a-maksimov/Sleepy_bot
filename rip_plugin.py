...
def rip(bot, message):
    ...
    if db:
        try:
            song = (audio_message.audio.file_id, audio_message.audio.performer + " - " + audio_message.audio.title,
            ...
            db_worker.close()
            bot.send_message(audio_message.chat.id,
                             "Song " + audio_message.audio.file_id + " added to database, thanks!",
                             reply_to_message_id=audio_message.message_id)
        except:
            bot.send_message(audio_message.chat.id, "Bad ID3 tags or duplicate.",
                             reply_to_message_id=audio_message.message_id)
